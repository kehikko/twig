...
function twig_init(array $extra_paths = [])
{
    ...
    $templates_vendor = tool_system_find_files(['views'], [cfg(['paths', 'vendor'])], 2, true);
    $templates_routes = tool_system_find_files(['views'], [cfg(['paths', 'routes'])], 1, true);
    $templates        = array_merge($templates_vendor, $templates_routes, $extra_paths);
    array_unshift($templates, cfg(['paths', 'views']));
